...
If your response was more to the tune of: "How does that work? Let's look it up!" Then I think you are on the track to greatness, keep it up. For the rest of us, sometimes you just need to move on and use a thing for awhile before you have the time and patience to dig through the innards of how it works.
...
Our function is fairly standard, excepting as the first parameter an array of strings, and the second any variable. The magic happens when it is invoked. Calling the function in this format:
```js
taggedTemplate``
```
...
taggedTemplate()
```
will cause our function to parse the string and separate it into a new array position as it encounters any variables within the string. This gives us a lot of power in creating custom strings based on conditions. Let's look at a little more dynamic example:
...
In this more complicated usage, we have the ability to write a string at the beginning optionally, and then list 2 variables. The function does some silly conditonals to return a string explaining the damage you have dealt. This could all be done differently using a standard function, but hopefully it shows how tagged templates handle strings and multiple variables.
...
Since we do not know how many variables will be passed to us, we'll spread them into an array called `variables`. I pondered for a while on how to place the variables in their appropriate spots in the final string ouput until I realized that it was quite simple. Tagged template strings are split on each encountered variable, meaning if you looped over the string array, your current index will match the index of the variable that caused the split.
...
We are near the home stretch now, hang on just a little bit longer. In the first example I showed for Styled Components they used function callbacks to access `props` and perform logic based on the prop. We are going to combine that with our theme in this last example.
...
There is obviously a great deal more going in Styled Components, such as the context it provides, but I believe this takes us far enough in understanding the underlying syntax of how it might use tagged templates to give us the powerful features it does. Going through this process relieved some anxiety I had about the magic going on behind the scenes, and I hope it has done the same for you. We'll see you next time as we continue to explore more Arcane Mysteries!